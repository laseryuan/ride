...
# if host user is root user we do not need to do anything
if [[ ${HOST_USER_NAME} == "root" || ${HOST_USER_ID} == 0} ]]; then
  echo "We don't map ride to root user." ; exit 0
fi
...
# chown -R ${RIDE_USER_ID}:${RIDE_USER_GID} ${RIDE_USER_HOME}
# export HOME=${RIDE_USER_HOME}
